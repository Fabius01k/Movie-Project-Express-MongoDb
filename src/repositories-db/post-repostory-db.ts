...
import {ClassPostDb, ClassPostsLikesInfoDb, PostsLikesInfo} from "../classes/posts/posts-class";
...
export class PostsRepostory {
    async findPosts(sortBy: string,sortDirection: 'asc' | 'desc',
        ...
        const items = await Promise.all(posts.map(p => mapPostFromDbView(p,userId)))
    ...
    async createPost(newPost: ClassPostDb,userId: string | null): Promise<TPostView | null> {

        await postsModel.insertMany([newPost])

        return mapPostFromDbView(newPost,userId)
    }
    async createPostByBlogId(newPost: ClassPostDb,userId: string | null): Promise<TPostView | null> {
...
*/
    ...
    async createCollectionOfPostsLikesInf(newCollectionPostsLikesInfo: ClassPostsLikesInfoDb):
    Promise<ClassPostsLikesInfoDb> {
        let result = await postsLikesInfoModel
            .insertMany([newCollectionPostsLikesInfo])
    ...
    async findOldLikeDislike(infoId: string,userId: string) {
        const result = await postsLikesInfoModel.findOne({ infoId ,"likesInfo.userId": userId})
        if (result?.likesInfo) {
    ...
    async updateNumberOfLikes(infoId: string, likeInfo: PostsLikesInfo): Promise<boolean> {
        const result = await postsLikesInfoModel.updateOne(
            { infoId },
            { $inc: { numberOfLikes: 1 },
    ...
    async updateNumberOfDislikes(infoId: string, likeInfo: PostsLikesInfo): Promise<boolean> {
        const result = await postsLikesInfoModel.updateOne(
            { infoId },
            { $inc: { numberOfDislikes: 1 },
                $push: { likesInfo: likeInfo }}
